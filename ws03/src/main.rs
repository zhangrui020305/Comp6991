...
fn run_command(cmd: &str, buffers: &mut HashMap<String, BufferEditor>)  -> Result<(), Box<dyn Error>> {
    ...
    if cmd.starts_with("open") {
        let buffer_name = cmd.split_once(" ").expect("Missing buffer name").1.to_string();
        let editor = buffers.entry(buffer_name).or_insert(BufferEditor {
            buffer: Buffer::new(),
        });
        run_game(
            editor,
        ...
        let target = cmd.split_once(" ").expect("Missing buffer name").1.to_string();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut buffer: HashMap<String, BufferEditor> = HashMap::new();
