...
fn handle_client(mut stream: TcpStream, state: Arc<Mutex<State>>) {
    ...
    let file_template = String::from_utf8_lossy(file);

    let counter_string;
    {
        let state_guard = state.lock().unwrap();
        counter_string = state_guard.counter.to_string();
    }

    let final_html_string = file_template.replace("{{{ counter }}}", &counter_string);
    let file = final_html_string.as_bytes();
